...
import { WorkLocationPropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkLocationPropsType & { onChange?: (v: WorkLocationPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, onChange, disabled } = props;
  const local = useMemo(() => ({ required }), [required]);
  const update = (patch: Partial<WorkLocationPropsType>) => onChange?.({ ...local, ...patch });
  return (
    <FieldGroup>
