...
import { WorkRankingPropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkRankingPropsType & { onChange?: (v: WorkRankingPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, onChange, disabled } = props;
  const local = useMemo(() => ({ required }), [required]);
  const update = (patch: Partial<WorkRankingPropsType>) => onChange?.({ ...local, ...patch });
  return (
    <FieldGroup>
