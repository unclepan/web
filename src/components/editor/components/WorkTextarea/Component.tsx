"use client";
import { FC } from "react";
import { Textarea } from "@/components/ui/textarea";
import { useEditor } from "@/components/editor/store/EditorProvider";
import WorkReactQuill from "../WorkReactQuill";
import { WorkTextareaPropsType, WorkTextareaDefaultProps } from "./interface";

const Component: FC<WorkTextareaPropsType & { feUuid?: string }> = (props) => {
  const { dispatch } = useEditor();
  const { title = "", placeholder, feUuid = "" } = { ...WorkTextareaDefaultProps, ...props };
  const handleChange = (editorProp: string, delta: string) => {
    if (!feUuid) return;
    dispatch({ type: "PUSH_PAST" });
    dispatch({ type: "CHANGE_COMPONENT_PROPS", payload: { feUuid, newProps: { title: delta } } });
  };
  return (
    <div>
      <WorkReactQuill value={title} editorProp="title" feUuid={feUuid} onChange={handleChange} />
      <div className="px-1 mt-1">
        <Textarea
          placeholder={placeholder}
          readOnly
          rows={3}
          className="pointer-events-none resize-none"
        />
      </div>
    </div>
  );
};
export default Component;
