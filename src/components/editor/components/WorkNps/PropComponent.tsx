...
import { WorkNpsPropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkNpsPropsType & { onChange?: (v: WorkNpsPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, onChange, disabled } = props;
  const local = useMemo(() => ({ required }), [required]);
  const update = (patch: Partial<WorkNpsPropsType>) => onChange?.({ ...local, ...patch });
  return (
    <FieldGroup>
