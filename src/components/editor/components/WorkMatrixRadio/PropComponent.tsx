...
import { WorkMatrixRadioPropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkMatrixRadioPropsType & { onChange?: (v: WorkMatrixRadioPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, onChange, disabled } = props;
  const local = useMemo(() => ({ required }), [required]);
  const update = (patch: Partial<WorkMatrixRadioPropsType>) => onChange?.({ ...local, ...patch });
  return (
    <FieldGroup>
