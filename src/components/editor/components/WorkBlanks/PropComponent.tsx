...
import { WorkBlanksPropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkBlanksPropsType & { onChange?: (v: WorkBlanksPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, onChange, disabled } = props;
  const local = useMemo(() => ({ required }), [required]);
  const update = (patch: Partial<WorkBlanksPropsType>) => onChange?.({ ...local, ...patch });
  return (
    <FieldGroup>
