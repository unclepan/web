...
import { WorkMaxDiffPropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkMaxDiffPropsType & { onChange?: (v: WorkMaxDiffPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, onChange, disabled } = props;
  const local = useMemo(() => ({ required }), [required]);
  const update = (patch: Partial<WorkMaxDiffPropsType>) => onChange?.({ ...local, ...patch });
  return (
    <FieldGroup>
