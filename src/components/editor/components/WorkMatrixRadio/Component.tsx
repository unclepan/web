...
import { WorkMatrixRadioPropsType, WorkMatrixRadioDefaultProps } from "./interface";
import { nanoid } from "nanoid";

const Component: FC<WorkMatrixRadioPropsType & { feUuid?: string }> = (props) => {
  const { dispatch } = useEditor();
  const { t } = useLocale();
  const { title = "", rows = [], columns = [], feUuid = "" } = { ...WorkMatrixRadioDefaultProps, ...props };

  const updateProps = (newProps: Partial<WorkMatrixRadioPropsType>) => {
    if (!feUuid) return;
  ...
  const addCol = () => updateProps({ columns: [...columns, { value: nanoid(5), text: format(t((m) => m.editor.canvasOptionN), { n: columns.length + 1 }) }] });
    ...
    <div>
                ...
                {columns.map((col) => (
                  <TableCell key={col.value} className="text-center">
                    <RadioGroup className="flex justify-center">
                      <RadioGroupItem value={`${row.value}-${col.value}`} id={`mradio-${row.value}-${col.value}`} />
                    </RadioGroup>
                  </TableCell>
                ))}
              </TableRow>
      ...
      <div className="mt-2 flex gap-2 border-t border-dashed border-border pt-2">
        ...
        <Button variant="outline" size="sm" onClick={addCol}>
          <Plus />{t((m) => m.editor.canvasAddOption)}
        </Button>
      </div>
    </div>
  );
