...
const PropComponent: FC<WorkRadioPropsType & { onChange?: (v: WorkRadioPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, row, onChange, disabled } = props;
  const local = useMemo(() => ({ required, row }), [required, row]);
  const update = (patch: Partial<WorkRadioPropsType>) => onChange?.({ ...local, ...patch });
  const rowOptions = [
