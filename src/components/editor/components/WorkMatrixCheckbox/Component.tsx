...
import { WorkMatrixCheckboxPropsType, WorkMatrixCheckboxDefaultProps } from "./interface";
import { nanoid } from "nanoid";

const Component: FC<WorkMatrixCheckboxPropsType & { feUuid?: string }> = (props) => {
  const { dispatch } = useEditor();
  const { t } = useLocale();
  const { title = "", rows = [], columns = [], feUuid = "" } = { ...WorkMatrixCheckboxDefaultProps, ...props };

  const updateProps = (newProps: Partial<WorkMatrixCheckboxPropsType>) => {
    if (!feUuid) return;
                ...
                {columns.map((col) => (
                  <TableCell key={col.value} className="text-center">
                    <div className="flex justify-center">
                      <Checkbox id={`mcheck-${row.value}-${col.value}`} />
                    </div>
                  </TableCell>
                ))}
              </TableRow>
