...
import { WorkImageRadioPropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkImageRadioPropsType & { onChange?: (v: WorkImageRadioPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, row, onChange, disabled } = props;
  const local = useMemo(() => ({ required, row }), [required, row]);
  const update = (patch: Partial<WorkImageRadioPropsType>) => onChange?.({ ...local, ...patch });
  const rowOptions = [
