...
import { WorkCascaderPropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkCascaderPropsType & { onChange?: (v: WorkCascaderPropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  const { required, onChange, disabled } = props;
  const local = useMemo(() => ({ required }), [required]);
  const update = (patch: Partial<WorkCascaderPropsType>) => onChange?.({ ...local, ...patch });
  return (
    <FieldGroup>
