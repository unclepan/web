...
import { WorkMatrixScalePropsType } from "./interface";
import { useLocale } from "@/i18n/useLocale";

const PropComponent: FC<WorkMatrixScalePropsType & { onChange?: (v: WorkMatrixScalePropsType) => void; disabled?: boolean }> = (props) => {
  const { t } = useLocale();
  ...
  const update = (patch: Partial<WorkMatrixScalePropsType>) => onChange?.({ ...local, ...patch });
    ...
    <FieldGroup>
      <div className="flex gap-2">
        <Field className="flex-1">
          ...
          <Input id="min" type="number" disabled={disabled} value={String(local.min ?? 1)} onChange={(e) => updateMin(Number(e.target.value))} />
        </Field>
        <Field className="flex-1">
          <FieldLabel htmlFor="max">{t((m) => m.editor.propMaxValue)}</FieldLabel>
          <Input id="max" type="number" disabled={disabled} value={String(local.max ?? 5)} onChange={(e) => updateMax(Number(e.target.value))} />
        </Field>
      </div>
      <Field>
        <FieldLabel htmlFor="minLabel">{t((m) => m.editor.propMinLabel)}</FieldLabel>
        <Input id="minLabel" disabled={disabled} value={local.minLabel || ""} onChange={(e) => update({ minLabel: e.target.value })} />
      </Field>
      <Field>
        <FieldLabel htmlFor="maxLabel">{t((m) => m.editor.propMaxLabel)}</FieldLabel>
        <Input id="maxLabel" disabled={disabled} value={local.maxLabel || ""} onChange={(e) => update({ maxLabel: e.target.value })} />
      </Field>
